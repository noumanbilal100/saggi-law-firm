      ...
      <section className="py-16 md:py-20">
        <div className="mx-auto max-w-[1240px] px-4 sm:px-6">
          <div className="mb-6 flex items-end justify-between gap-6">
            ...
            <span className="text-[0.95rem] font-semibold uppercase tracking-[0.12em] text-muted">
          ...
          <div className="flex flex-wrap gap-2.5">
            {alsoLocations.map((l) => (
              <Link
                key={l.slug}
                href={`/location/${l.slug}`}
                className="inline-flex items-center gap-1.5 rounded-full border border-rule bg-paper px-4 py-2 text-[1.02rem] font-medium text-ink transition-all hover:border-rust hover:bg-rust hover:text-white"
              >
                {l.name}
              </Link>
            ))}
